...
3. Env variablene som er lagt inn i naiserator-filene skal defineres i [config.ts](/server/src/config.ts)
   ...
   ````
      **environment** -> Her setter du hvilken miljøer som frontend er tilgjengelig i. Hvis den f.eks kun er tilgjengelig i dev (q1) trenger du ikke definere `environment:`. Men skal du tilgjengeliggjøre frontend i prod må du sette `environment: [Environment.PROD],` og eventuelt flere miljøer i samme array.
